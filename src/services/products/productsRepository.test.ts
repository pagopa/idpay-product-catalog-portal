...
describe('getEligibleProducts', () => {
  ...
  it('maps and filters products using adapter and returns mapped list', async () => {
    ...
    ;(getInitiativeConfig as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
      datasetFile: 'dataset.json'
    })

    const adapter = vi.fn(
      (p: { id: number; name: string; valid: boolean }) =>
        p.valid ? { id: p.id, name: p.name } : null
    )
    ;(getInitiativeAdapter as unknown as ReturnType<typeof vi.fn>).mockReturnValue(
      adapter
    )

    ;(fetchWithResilience as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(
      new Response(JSON.stringify([{ id: 1, name: 'A', valid: true }, { id: 2, name: 'B', valid: false }]), { status: 200 })
    )

    ;(structuralValidation as unknown as ReturnType<typeof vi.fn>).mockImplementation(
      (x: unknown) => x
    )
