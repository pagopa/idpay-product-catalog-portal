...
const initiatives = Object.fromEntries(
  [decoder, valore, elettrodomestici].map((config) => [
...
export const getAppConfig = (
  ...
  if (!Object.hasOwn(initiatives, id)) {
    throw new Error(
      `VITE_INITIATIVE must be one of: ${Object.keys(initiatives).join(
        ', ',
      )}. Received: "${id}"`,
    );
  }
  return { ...initiatives[id], basePath: `/${id}/elenco-prodotti/` };
};
