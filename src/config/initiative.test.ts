...
import { getInitiativeAdapter, getInitiativeConfig, INITIATIVE_NAME } from './initiativeResolver';

describe('initiative build configuration', () => {
  it.each(['bonusdecoder', 'bonusvalore', 'bonuselettrodomestici'])('resolves the deployment path and dataset for %s', id => {
    const config = getAppConfig(id, false);
    expect(config.basePath).toBe(`/${id}/elencoprodotti/`);
    expect(config.datasetFile).toBe(`product_export_${config.initiativeId}.json`);
  ...
  it.each(['unknown', 'toString', '__proto__'])('rejects unsupported initiative %s', id => {
